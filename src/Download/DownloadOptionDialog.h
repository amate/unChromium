...
struct CDLOptions
{
	...
	static void	LoadProfile()
	{
		...
		strSEFilePath		= pr.GetString(_T("SEFile"));
	...
	static void	SaveProfile()
	{
		...
		pr.SetString(strSEFilePath, _T("SEFile"));
	...
	static void PlaySE()
	{
		if (strSEFilePath.IsEmpty() == FALSE)
			::PlaySound(strSEFilePath, NULL, SND_ASYNC | SND_FILENAME);
	}
...
__declspec(selectany) CString	CDLOptions::strSEFilePath;
...
public:
    ...
    BEGIN_DDX_MAP(CDLOptionDialog)
		...
		DDX_TEXT(IDC_EDIT_SEFILE				, strSEFilePath	)
    END_DDX_MAP()
	...
	BEGIN_MSG_MAP(CDLOptionDialog)
		...
		COMMAND_ID_HANDLER_EX(IDC_BUTTON_SEFILESELECT, OnSEFileSelect )
		COMMAND_ID_HANDLER_EX(IDC_BUTTON_PLAYSE, OnPlaySE )
	END_MSG_MAP()
	...
	void OnSEFileSelect(UINT uNotifyCode, int nID, CWindow wndCtl)
	{
		CString strPath;
		COMDLG_FILTERSPEC filter[] = {
			{ L"サウンドファイル (*.wav)", L"*.wav" },
			{ L"すべてのファイル (*.*)", L"*.*" }
		};
		CShellFileOpenDialog	SHdlg(NULL, FOS_FORCEFILESYSTEM | FOS_PATHMUSTEXIST | FOS_FILEMUSTEXIST, NULL, filter, _countof(filter));
		if (SHdlg.IsNull() == false) {
			if (SHdlg.DoModal(m_hWnd) == IDOK)
				SHdlg.GetFilePath(strPath);
		} else {
			...
			if (dlg.DoModal(m_hWnd) == IDOK) {
				dlg.GetFilePath(strPath.GetBuffer(MAX_PATH), MAX_PATH);
				strPath.ReleaseBuffer();
			}
		}
		if (strPath.IsEmpty() == FALSE) {
			strSEFilePath = strPath;
			DoDataExchange(DDX_LOAD, IDC_EDIT_SEFILE);
		}
	}

	void OnPlaySE(UINT uNotifyCode, int nID, CWindow wndCtl)
	{
		PlaySE();
	}
