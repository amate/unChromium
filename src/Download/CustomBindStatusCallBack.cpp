...
/// pDLItem->strFilePath と m_pDLItem->strIncompleteFilePath を埋める
// pDLItem->strFileName をデフォルトのファイル名に使うのであらかじめ埋めておく
// @return	DLを開始するなら true 、ユーザーにキャンセルされたらfalseを返す
bool	GetFileName(DLItem* pDLItem)
{
	...
	if (CDLOptions::bUseSaveFileDialog || ::GetKeyState(VK_CONTROL) < 0) {
		...
		if (ShellSaveFileDialog.IsNull()  == false) {
			...
			CFileDialog dlg(FALSE, NULL, pDLItem->strFileName, OFN_OVERWRITEPROMPT, _T("すべてのファイル (*.*)\0*.*\0\0"));
